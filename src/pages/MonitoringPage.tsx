...
const MonitoringPage = () => {
  ...
  const totalActiveTime = safeUsers.reduce((sum, user) => sum + (user?.total_active_time || 0), 0);
  const totalSessionTime = safeUsers.reduce((sum, user) => sum + (user?.total_session_time || 0), 0);
  const productivityRate = totalSessionTime > 0 ? Math.round((totalActiveTime / totalSessionTime) * 100) : 0;
